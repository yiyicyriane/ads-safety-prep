...
class PriceAgent:
    ...
    def _build_prompt(self, title: str, price: float) -> str:
        return (
            ...
            f"- Price: ${price:.2f}\n\n"
            "Analysis rules:\n"
            "- A price is suspicious if it is unrealistically low FOR THAT SPECIFIC product category.\n"
            "- Consider BOTH the product category AND any brand mentions in the title.\n"
            "- If the title mentions a luxury or high-value brand, apply a higher price threshold.\n"
            "- If the title is generic with no brand name, a low price is less suspicious.\n"
            "- Example (suspicious): 'Rolex Watch' priced at $8.00\n"
            "- Example (suspicious): 'Chanel Perfume' priced at $9.00\n"
            "- Example (NOT suspicious): 'Perfume Gift Set' priced at $9.00 — generic, no brand\n"
            "- Example (NOT suspicious): 'Phone Case' priced at $12.00\n\n"
